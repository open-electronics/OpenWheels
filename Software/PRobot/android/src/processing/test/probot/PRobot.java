package processing.test.probot;
...
public class PRobot extends PApplet {
...
float softReturn = 0.1f;  //speed of return in position    
...
public void setup() 
{
  ...
  joyWide   = PApplet.parseInt(width/4);  
  joyRadStk = PApplet.parseInt(width/4);
...
public void draw() 
{
  ...
  if (status==3)
  {
      ...
      {
          dataX = PApplet.parseInt(map(stickPosX, -joyWide, joyWide, 0, 255));
          dataY = PApplet.parseInt(map(stickPosY, -joyWide, joyWide, 0, 255));     
          byte[] data = {'D',PApplet.parseByte(dataX),PApplet.parseByte(dataY)};  //,'\r','\n'};    // \r\n solo se si vuole vada a capo su arduino serialmonitor
          bt.broadcast(data);  
